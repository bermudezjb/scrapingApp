...
// url= https://www.tutellus.com/buscador/react/cursos
...
const extractProductData = async (url,browser) => {
    ...
    // -Imagen
        ...
        //titulo
        courseData['name'] = await page.$eval('.title', title=>title.innerText)
        //¿Quien lo hace?
        courseData['author'] = await page.$eval('div.content a span', author=>author.innerText)
        courseData['author2'] = await page.$eval('div.content a span:last-child', author=>author.innerText)
        // Horas del curso
        courseData['duration'] = await page.$eval('.softblack', duration=>duration.innerText)
        // Rating
        courseData['rating'] = await page.$eval('div.mtm > div:nth-child(5) > strong', rating=>rating.innerText)
        // Nivel
...
const scrap = async (url) => {
    try {
        ...
        const tmpurls = await page.$$eval('.course-title a',data=>data.map(product=>product.href))
